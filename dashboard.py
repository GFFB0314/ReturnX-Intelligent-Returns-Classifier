...
# Page configuration
st.set_page_config(
    page_title="ReturnX - Returns Classifier", page_icon="📦", layout="wide"
)
...
# Header
st.markdown(
    '<div class="main-header">📦 ReturnX Intelligent Returns Classifier</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">Automated routing of product returns using Machine Learning</div>',
    unsafe_allow_html=True,
)
...
except (IOError, EOFError, ValueError, TypeError) as e:
...
with st.sidebar:
    ...
    example_type = st.selectbox(
        "Load example:", ["Select...", "Defect", "Sizing", "Style", "Other"]
    )
...
with col1:
    ...
    examples = {
        "Defect": {
            "text": (
                "The zipper broke after one use and there's a hole in the fabric. "
        ...
        "Sizing": {
            "text": (
                "The shirt is too small. I ordered a large but it fits like a medium. "
                "Runs very small."
            ),
            "age": 35,
            "rating": 3,
        ...
        "Style": {
            "text": (
                "The color doesn't match the picture at all. It's much darker and "
    ...
    with col_age:
        age = st.number_input(
            "Customer Age:", min_value=18, max_value=99, value=default_age, step=1
        )

    with col_rating:
        rating = st.selectbox(
            "Product Rating:", options=[1, 2, 3, 4, 5], index=default_rating - 1
        )

    # Predict button
    predict_button = st.button(
        "🔍 Classify Return", type="primary", use_container_width=True
    )
